...
describe('AddUserModal', () => {
  const props = {
    closeModal: jest.fn(),
    isOpen: true,
    handleAddUserSubmit: jest.fn(),
  };

  beforeEach(async () => {
    await act(async () => {
      await render(<AddUserModal {...props} />);
    });
  });
  ...
  describe('Initial Render', () => {
    it('should load modal with title Add User', () => {
      const title = screen.getAllByText('Add User')[0];
    ...
    it('should load modal with button Add User', () => {
      const submitBtn = screen.getAllByText('Add User')[1];
  ...
  describe('Add User', () => {
    it('should call handleAddUserSubmit', async () => {
      await act(async () => {
      ...
      expect(props.handleAddUserSubmit).toHaveBeenCalledTimes(1);
    });
  });
});
